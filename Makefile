.PHONY: all clean calcstats worker genconfigs operator runexperiments build syncbucket

all: clean build

clean:
	go clean -i ./...
	rm -rf calcstats worker genconfigs operator runexperiments

build: calcstats worker genconfigs operator runexperiments

calcstats:
	CGO_ENABLED=0 go build -a -ldflags '-w -extldflags "-static"' ./cmd/calcstats

worker:
	CGO_ENABLED=0 go build -a -ldflags '-w -extldflags "-static"' ./cmd/worker
...
syncbucket:
	...
	gsutil cp ${GOPATH}/src/github.com/numbleroot/zeno-eval/worker gs://acs-eval/worker
